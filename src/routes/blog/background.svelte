...
import React, { FC } from 'react';
    ...
    const Star: FC<StarProps> = (props) => {
      const { giantColor, starColor, cometColor, starDensity, speedCoeff } = props;
      let universe: CanvasRenderingContext2D;
      let width: number;
      let height: number;
      ...
      let first = true;
      let canva = document.getElementById('universe') as HTMLCanvasElement;
      let stars: Star[] = [];
      ...
      function createUniverse() {
        universe = canva.getContext('2d') as CanvasRenderingContext2D;
